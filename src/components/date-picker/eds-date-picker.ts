 ...
 *
 * @fires eds-change - Fired when the selected date changes. Detail: `{ value: string }` (YYYY-MM-DD).
 *
 * @element eds-date-picker
 */
@customElement('eds-date-picker')
export class EdsDatePicker extends LitElement {
  ...
  private readonly fieldId = `eds-date-picker-${Math.random().toString(36).slice(2, 9)}`;
      ...
      >
        ...
        <div class="days" role="grid" aria-label=${formatMonthYear(this.viewYear, this.viewMonth)}>
          ${days.map((cell) => {
            ...
            const selected = cell.iso === this.value;
  ...
  override render() {
    ...
    const describedBy =
      [this.invalid && this.errorMessage ? errorId : '', this.hint ? hintId : '']
        .filter(Boolean)
        .join(' ') || undefined;
    ...
    return html`
      <div class="field">
        ...
        ${this.invalid && this.errorMessage
          ? html`<div class="error" id=${errorId} role="alert">${this.errorMessage}</div>`
          : nothing}
        ${this.hint
